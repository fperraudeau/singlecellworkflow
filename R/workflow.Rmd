...
```{r}
se <- SummarizedExperiment(assays = list(rawCounts = core), colData = metadata)
```
...
if (runZinb){
  ...
  # K = 0
  print(system.time(zinb0 <- zinbFit(core, ncores = NCORES, K = 0, X = mod)))
  save(zinb0, file = f50)
  # K = 50
  print(system.time(zinb50 <- zinbFit(core, ncores = NCORES, K = 50, X = mod)))
...
if (runClus){
  print(system.time(ceObj <- RSEC(t(W), k0s = 4:15, 
                                  ...
                                  mergeMethod = "locfdr", mergeCutoff = 0.01)))
  save(ceObj, file = fn)
}else{
  load(fn)
}
...
mergeClusters(ceObj, mergeMethod = "locfdr", plotInfo="mergeMethod",cutoff = 0.01)
```

```{r heatmap }
plotHeatmap(ceObj, clusterSamplesData="dendrogramValue", breaks = .99)
```

```{r compareClusters}
plot(W, col = col_clus[as.character(clus.labels)], main = 'color = previous', pch = 20)
plot(W, col = as.factor(primaryCluster(ceObj)), main = 'color = new', pch = 20)
...
plotHeatmap(res, Colv = merged@dendro_samples,
