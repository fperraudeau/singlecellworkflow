...
if(packageVersion("zinbwave")<'0.99.4.1') stop("must have current develop version to avoid bugs")
```
...
if (runZinb & !file.exists(fn)){
  ...
  print(system.time(zinb <- zinbDimRed(se, K = 50, X = mod, residuals = T,
                                       normalizedValues = T)))
  save(zinb, file = fn)
}else{
  load(fn)
...
```{r}
res <- computeDevianceResiduals(zinb, core)
res[1:3,1:3]
```
