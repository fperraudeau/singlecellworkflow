...
library(rARPACK)
set.seed(20)
if(packageVersion("clusterExperiment")<'1.3.0.9008') stop("must have current develop version to avoid bugs")
...
FP: for zinbwave, parallel computing is now handle by BiocParallel. If you have a Windows machine, please update the code to allow paralell computing.
...
FP: Elizabeth, I could not install the last version 1.3.0.9009 of clusterExperiment. The error is
...
if (runZinb & !file.exists(fn)){
  print(system.time(zinb <- zinbDimRed(se, K = 50, X = '~ batch',
                                       residuals = T,
                                       normalizedValues = T)))
  save(zinb, file = fn)
}else{
...
```{r norm}
res <- assays(zinb)[['normalizedValues']]
res[1:3,1:3]
```
