...
Metadata for the cells are stored in the object colData from the SummarizedExperiment object. Cells were processed in 18 different batches.
...
table(clus.labels)
```
Note that there is partial confounding of batches and clusters, which could be problematic when correcting for batch effects in the dimensionality reduction step.
