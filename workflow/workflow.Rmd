...
- install and run package making sure we can build the vignette using R 3.4 and bioc-devel 3.6 (all?),
...
```{r barplotOurs, fig.cap="RSEC: Barplot of number of cells per cluster, for our workflow's clustering, color-coded by RSEC clustering."}
...
[FP: Elizabeth or Davide, how can we change the colors in Figure \@ref(fig:addPublishedClusters) to match the colors we use for publishedClusters (that is `col_clus` like in Figure \@ref(fig:mdsW)). I tried colPalette argument, but it does not work with SummarizedExperiment input. Also, would it be possible to have the same order and labels for the barplots in Figures \@ref(fig:barplotOurs) and \@ref(fig:addPublishedClusters) (barplots labelled -1 and Not Assigned are the same, right?)?

```{r addPublishedClusters, fig.cap="RSEC: Barplot of number of cells per cluster, for our workflow's clustering, color-coded by original published clustering."}
...
```{r mdsWce, fig.cap="RSEC: MDS of the low-dimensional matrix W, where each point represents a cell and cells are color-coded by RSEC clusters."}
...
c7 | MV | cian | original 15

Cells in cluster `c4` have a cluster label of `-2` in the original published clustering meaning that they were not assigned to any cluster in the original publication. These cells were actually identified as non-sensory contaminants as they overexpress gene `Reg3g` (see Figure S1 from [@Fletcher2017] and Figure \@ref(fig:boxplotReg3g)) and were removed from the original published clustering. While it is reassuring that our workflow clustered these cells separately with no influence on the clustering of the other cells, we removed cluster `c4` to infer lineages and pseudotimes as cells in this cluster do not participate in the cell differentiation process. Note that our workflow's clustering did not capture all the cells overexpressing gene `Reg3g` in cluster `c4` (see Figure \@ref(fig:boxplotReg3g)), however we keep these cells to infer lineages as they did not seem to influence the clustering.

[FP: check writing + table with correspondance of clusters]

```{r boxplotReg3g, fig.width=4, fig.height=4, fig.cap="Boxplots of the log counts of gene Reg3g for cells in cluster c4 (left boxplot) and cells not in cluster c4 (right boxplot)."}
c4 <- rep("other clusters", ncol(se)) 
c4[primaryClusterNamed(ceObj) == "c4"] <- "cluster c4"
boxplot(log1p(assay(se)["Reg3g", ]) ~ c4, ylab = "log counts", cex.axis = .8, cex.lab = .8)
```
