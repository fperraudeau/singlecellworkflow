...
- add details in section preprocessing (DR), DONE,
- address Sandrine's comments in sections slingshot and de (KS),
- try running workflow with more genes (FP),
- plot gene variance density (FP),
...
of the Bioconductor packages `scone (>=1.1.2)`, `zinbwave (>=0.99.5)`, and `clusterExperiment (>=1.3.2)` [DR: Elizabeth, please check!]. We recommend running Bioconductor 3.6 (currently the devel version; see https://www.bioconductor.org/developers/how-to/useDevel/).
...
```{r zinb}
print(system.time(se <- zinbwave(core, K = 40, X = "~ Batch",  
                                 residuals = TRUE,
...
[FP: I tried to play with the epsilon to see if we could get rid of the infinite values in the normalized values. With default epsilon, we have about 40 infinite values, with epsilon 10000, we have about 20. When K = 0, there is no infinite values but the normalized values don't capture as much of biology (looking at the pca with published clusters colors). It seems to me that we get infinite norm. values when the number of parameters to estimate is large. I tried to reduce K to 25 (no infinite but W not as good), 40 (infinite values but W looks ok)... to be continued.]
...
Before fitting the simultaneous principal curves, we check the structure of the lineages by plotting the MST on the clusters. This shows that our implementation has recovered the lineages found in the published work (Figure \@ref(fig:tree)). The `slingshot` package also includes methods for 3-dimensional visualizations as in Figure \@ref(fig:stemcelldiff), using the `plot3d` function from the package `rgl`.
