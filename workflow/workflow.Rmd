...
author: 
  ...
  - name: Sandrine Dudoit
    affiliation: Division of Epidemiology and Biostatistics and Department of Statistics, University of California, Berkeley, Berkeley, CA
...
One of the cornerstones of Bioconductor is the use of common data containers, implemented as classes in the S4 object-oriented system of R [@huber2015orchestrating].
...
Note that the pre-processing steps needed to produce such a count matrix from the raw FASTQ files are outside of the scope of this paper. These are very important steps that should be taken with care and the optimal workflow depends on the technology and the protocol used to collect the data (e.g., 3'-end sequencing, droplet-based methods versus full-length mRNA, plate-based methods). 
One way to carry out these steps within the Bioconductor framework is the `scPipe` package [@tian2017scpipe], which provides functions to quantify gene expression starting from raw sequencing files. More details can be found in the `scPipe` vignette and in [@tian2017scpipe].

## Package installation and versions

The source code to reproduce the analysis in the workflow is available at https://github.com/fperraudeau/singlecellworkflow. The packages used in the workflow can be installed using the following in R:
...
As discussed above, we store the data in an object of class `SingleCellExperiment`. In addition to keeping the expression values and their associated metadata within a single object, `SingleCellExperiment` allows the user to store normalization factors and the results of dimensionality reduction methods (such as `zinbwave`) within the same object. Throughout the workflow, we will highlight when this special features of `SingleCellExperiment` are used.
...
There are three special sets of genes in this dataset: the CreER gene corresponds to the estrogen receptor fused to Cre recombinase (Cre-ER), which is used to activate HBCs into differentiation following injection of tamoxifen (see [@Fletcher2017] for details); the Sox2-eGFP transgene was used for FACS sorting; the ERCC spike-in sequences are synthetic genes spiked-in at known concentrations and used for quality control purposes.

We can specify spike-in genes within the `SingleCellExperiment` class. In this way, it is easier to retrieve their expression measures. In addition, some specific functions may make direct use of this information (see, e.g., the `computeSpikeFactors` function in the `scran` package).

In this case, we specify two different sets of spike-ins, the ERCC spike-ins and the transgenic genes (eGFP and Cre-ER). This is a slight abuse of terminology, as the CreER and eGFP genes are not exactly spike-ins, but it is nonetheless useful to specify that these are not endogenous genes.

```{r}
# Mark ERCC and CreER genes as spike-ins
isSpike(dataObj, "ERCC") <- grepl("^ERCC-", rownames(dataObj))
isSpike(dataObj, "Transgene") <- grepl("^CreER$", rownames(dataObj)) | 
                                  grepl("^eGFP$", rownames(dataObj)) 
```
...
```{r spikebatch, fig.cap="Distribution of ERCC spike-in read counts per batch."}
boxplot(colData(dataObj)$ERCC_reads~batch, col = col_batch,
        outline = 0, ylab="Reads mapped to ERCC", las = 3)
```

Figure \@ref(fig:spikebatch) shows how for some batches no reads were mapped to ERCC spike-ins, presumably because the spike-ins were not included. Since the utility of the spike-ins is limited in this example, we remove the spike-ins from our object.
...
In the original work [@Fletcher2017], cells were clustered into 14 different clusters, with 233 cells not assigned to any cluster (i.e., cluster label of `-2`). 
...
See the [scone vignette](https://www.bioconductor.org/packages/release/bioc/vignettes/scone/inst/doc/sconeTutorial.html) for details on the filtering procedure.
...
Often in unsupervised analyses of expression data, it is useful to focus on the genes that vary the most in expression across cells and are thus likely to be the most informative. Filtering out low-variance genes allows us to reduce the complexity of the problem, while retaining the most information in the dataset.

Here, for computational efficiency, we retain only the 1,000 most variable genes, following a simple `log(x+1)` transformation. This seems to be a reasonable choice for the illustrative purpose of this workflow, as we are able to recover the biological signal found in the published analysis ([@Fletcher2017]). In general, however, we recommend care in selecting a gene filtering scheme, as an appropriate choice is dataset-dependent and many different strategies have been proposed in the literature. 

To compute the gene-wise variances and keep only the most variable genes, we use two functions from the `clusterExperiment` package. In particular, `makeFilterStats` will compute the variance of the gene expression data with a given transformation applied to the counts (in this case `log(x+1)`) and store the variance values in the `rowData` slot of the resulting `SingleCellExperiment` object. `filterData` will then use the computed values to keep only the desired number of genes.
...
One drawback of this strategy is that the logarithm is not a variance-stabilizing transformation. Variance-stabilizing transformations based on the negative binomial distribution are available, e.g., in the `DESeq2` package, and may be a better choice in real data analyses. Note that the `makeFilterStats` function of `clusterExperiment` can be used with any user-supplied function. Here, we show an example with the `vst` function from `DESeq2`.
...
Alternatively, the `scran` package [@Lun2016] includes the `trendVar` and `decomposeVar` functions, which estimate the mean-variance relationship and the technical and biological components of the variance, to select those genes with a high biological variance. See [@Lun2016] for details.
...
```{r zinbschema, echo=FALSE, out.width="95%", fig.cap="ZINB-WaVE: Schematic view of the ZINB-WaVE model. This figure was reproduced with kind permission from Risso et al. (2018)."}
knitr::include_graphics("zinb_schema.png")
```

The model can be thought of as a generalization of a factor analysis model, in which the biological signal of interest is assumed to be explained by a small number of "factors", represented in the model by the matrix `W` (Figure \@ref(fig:zinbschema)).
...
We set the normalized values as our first assay, so that they become the default in the plots below. Note that one can specify which assay to plot with the `whichAssay` parameter in the `clusterExperiment` visualization functions.
...
The most important parameter to choose for the dimensionality reduction step is the number of hidden covariates to be included in the model, i.e., the number of columns of the hidden matrix `W`, which we denote by `K`. As with any dimensionality reduction technique, choosing the number of factors `K` is far from trivial. There is no "one-size-fits-all" solution and the researcher may want to try a few different values of `K` and compare the results.

The `zinbwave` package provides two functions that may be used to guide the choice of `K`. The functions `zinbAIC` and `zinbBIC`, which compute the Akaike information criterion (AIC) and the Bayesian information criterion (BIC), respectively, can be used to compare models with different values of `K` (smaller values of AIC and BIC indicate a better fit).

Here, we fit a few different models corresponding to different values of `K` using the `zinbFit` function, which, unlike `zinbwave`, returns the estimates of all the parameters of the model as well as the value of the likelihood function.
...
```{r bic, fig.cap="ZINB-WaVE: BIC of model for different values of K."}
k_values <- c(1:5, 10)
...
```{r aic, fig.cap="ZINB-WaVE: AIC of model for different values of K."}
plot(k_values, sapply(fit, zinbAIC, t(counts(dataObj))),
     ylab = "AIC", xlab = "K", type = "b")
```

In this case, the BIC suggests a value of `K = 3` (Figure \@ref(fig:bic)), while the AIC suggests a value of `K >= 10` (Figure \@ref(fig:aic)).

Depending on the application and on the interpretation of the latent space, we may want to include different numbers of hidden factors. Here, the goal is for the reduced space to capture the biological signal of interest; selecting a large number of factors therefore seems preferable. The ZINB-WaVE model can also be used to infer unknown normalization factors (similar to RUV [@risso2014normalization]); in such a case, a smaller value of `K` would be more appropriate.

Finally, note that since ZINB-WaVE is linear in the number of genes and the number of cells, but quadratic in `K`, fitting multiple models and comparing their goodness-of-fit may not be feasible for large datasets. In such cases, choosing a single value of `K` may be needed. In practice, we found that `K` between 10 and 50 are reasonable choices in real datasets, when the goal is dimensionality reduction.
...
As one can see from the code above, there are many parameters that a user can set in RSEC. Many of the values that we choose are the default values of the function, but we nonetheless specify them for clarity and reproducibility. Here, we briefly describe the role of the parameters that we set, but a more comprehensive explanation is provided in the function's man page (`?RSEC`) and in [@Risso2018].
...
```{r examinemakeConsensus, fig.cap="RSEC: Candidate clusterings found using the function RSEC from the clusterExperiment package. The top row corresponds to the consensus clustering and is color-coded accordingly. The other rows correspond to the individual clusterings across which the consensus was computed. In each row, the colors are chosen to match as closely as possible with the colors of clusters with the largest overlap from the previous row."}
plotClusters(dataObj, existingColors = "firstOnly")
```

The `plotCoClustering` function produces a heatmap of the co-clustering matrix, which records, for each pair of cells, the proportion of times they were clustered together across the candidate clusterings (Figure \@ref(fig:plotcoclust)). 
...
Finally, we can visualize the cells in a two-dimensional space using the MDS of the low-dimensional matrix `W` and coloring the cells according to their newly-found RSEC clusters (Figure \@ref(fig:mdsWce)); this is analogous to Figure \@ref(fig:mdsW) for the original published clusters.
...
Here, we show how to use `slingshot` starting from a matrix of (low-dimensional) expression measures and a set of cluster labels.
...
Having found the global lineage structure, `slingshot` then constructs a set of smooth, branching curves in order to infer the corresponding pseudotime variables. Simultaneous principal curves are constructed from the individual cells along each lineage, rather than the cell clusters. During this iterative process, a cell may even be reassigned to a different lineage if it is significantly closer to the corresponding curve. This makes `slingshot` less reliant on the original clustering and generally more stable. The final curves are shown in Figure \@ref(fig:curves).
...
Generalized Additive Models (GAMs) [@gam] are a class of models that replaces the usual linear function of the coefficients with a sum of smooth functions. In the code below, we use a special case of the GAM in which only one covariate (pseudotime) is present. This corresponds to a simple loess regression, but with the added advantage of providing a framework for testing the significance of the association between pseudotime and expression.
...
Many different methods have been proposed for the analysis of scRNA-seq data and this paper presents only one possible workflow, using tools that we have developed in recent years. The use of the `SingleCellExperiment` class, which is the shared data container among Bioconductor's scRNA-seq software packages, makes it extremely easy to replace one of the steps above by a different approach. For instance, one could use `scPipe` [@tian2017scpipe] for gene expression quantification, `scater` [@McCarthy2017] for quality control and filtering, `scran` [@lun2016pooling] to robustly calculate global-scaling factors for normalization, and `SC3` [@kiselev2017sc3] for clustering, with minimal changes to the code structure above. This makes the present workflow a truly modular approach, in which a user can employ their favorite tool for each step with the guarantee that the output of each step will be readable by the next step.

An alternative normalization strategy is to compute gene and cell-specific scaling factors (e.g., `SCNorm` [@bacher2017scnorm]). Similarly, `monocle` [@trapnell2014dynamics] or `TSCAN` [@ji2016tscan] are alternatives for cell lineage inference. Finally, the software suites `Seurat` [@seurat] (available on CRAN) and `scde` [@kharchenko2014bayesian] provide alternative pipelines for cell type discovery, differential expression, and gene set analysis. A comprehensive list of tools developed for the analysis of scRNA-seq data is available at https://www.scrna-tools.org [@zappia2018exploring]. 

## Further developments

Thanks to the availability of droplet-based protocols, such as Drop-Seq and 10X Genomics Chromium, datasets made of tens of thousands of cells are now routine. Indeed, big collaborative projects such as the Human Cell Atlas [@regev2017science] and the BRAIN Initiative Cell Census Network [@ecker2017brain] will soon generate millions, if not billions, of cells. Hence, there is a growing need for scalable methods.

When datasets are too large to be fully stored in memory, HDF5 files [@folk2011overview] can be used to keep the data on disk and realize in memory only the subset of data needed at each step. We are in the process of updating the `clusterExperiment` and `zinbwave` packages to work with HDF5 files and we are developing more scalable methods to deal with massive datasets.

We are also developing a downstream method to identify genes that are DE either within or between lineages inferred from Slingshot. Whereas the GAM presented here can only identify dynamic genes within a single lineage, many researchers may be interested in finding genes that exhibit different expression patterns between lineages or identifying those genes which differentiate the earliest. For questions such as these, we are developing new, spline- and negative binomial-based methods for comparing gene dynamics between branching trajectories.
...
The authors are grateful to Professor John Ngai (Department of Molecular and Cell Biology, UC Berkeley) and his group members Dr. Russell B. Fletcher and Dr.Diya Das for motivating the research presented in this workflow and for valuable feedback on applications to biological data. We would also like to thank Michael B. Cole for his contributions to `scone`.
