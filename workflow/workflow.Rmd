...
keywords: single-cell, single-cell genomics, RNA-seq, gene expression, zero inflation, normalization, dimensionality reduction, clustering, lineage inference, differential expression, workflow, R, Bioconductor
...
[FP: we need to have only 8 keywords, which ones do you prefer?]
...
```{r barplotOurs, fig.cap="Barplot of number of cells per cluster found using our workflow."}
plotBarplot(ceObj) 
```
...
```{r addPublishedClusters, fig.cap="Barplot of number of cells per cluster in original published clusters."}
...
```{r comparePublished, fig.cap="Comparison between the clusters found using our workflow and original published clusters."}
...
```{r curves, fig.cap="Cells color-coded by cluster in a 4-dimensional space with smooth curves representing each inferred lineage."}
pairs(lineages, type='curves', pal[primaryClusterNamed(ceObj)])
```

```{r tree, fig.cap="Cells color-coded by cluster in a 4-dimensional space with connecting lines between cluster centers representing the inferred structure."}
