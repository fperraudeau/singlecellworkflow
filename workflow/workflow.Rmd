...
There are three special sets of genes in this dataset: the CreER gene corresponds to the estrogen receptor fused to Cre recombinase (Cre-ER), which is used to activate HBCs into differentiation following injection of tamoxifen (see [@Fletcher2017] for details); the Sox2-eGFP transgenic was used for FACS sorting; the ERCC spike-in sequences are synthetic genes spiked-in at known concentrations and used as quality controls.

We can specify spike-in genes withing the `SingleCellExperiment` class. In this way, it is easier to retrieve their expression. In addition, some specific functions may make direct use of this information (see, e.g., the `computeSpikeFactors` function in the `scran` package).

In this case, we specify two different sets of spike-ins, the ERCC spike-ins and the transgenic genes (eGFP and Cre-ER). This is a slight abuse of notation, as the CreER and eGFP genes are not exactly spike-ins, but it is nonetheless useful to specify that this is not an endogenous gene.
...
```{r spikebatch, fig.cap="Distribution of reads mapped onto ERCC spike-ins per batch."}
boxplot(colData(dataObj)$ERCC_reads~batch, col = col_batch,
        outline = 0, ylab="Expression measure", las = 3)
```

Figure \@ref(fig:spikebatch) shows how for some batches no reads are mapped to ERCC spike-ins, presumably because the spike-ins were not included. Since the utility of the spike-ins is limited in this example, we remove the spike-ins from our object.
...
DR: There is quite a difference between AIC and BIC in this case, and it is actually hard to argue one value over another... how should we discussed these results? Also, if I keep going AIC will keep decreasing...
...
As one can see from the code above, there are many parameters that a user can set in RSEC. Many of the values that we choose are the default values of the function, but we nonetheless specify them for clarity and reproducibility. Here, we briefly describethe role of the parameters that we set, but a more comprehensive explanation is provided in the function's man page (`?RSEC`) and in [@Risso2018].
...
- `consensusProportion`: the proportion of times that two sets of samples should be together in order to be grouped into a cluster.
...
Thanks to the availability of droplet-based protocols, such as Drop-Seq and 10X Genomics Chromium, it is now becoming routine to generate datasets made of tens of thousands of cells. In fact, big collaborative projects such as the Human Cell Atlas [@regev2017science] and the BRAIN Initiative Cell Census Network [@ecker2017brain] will soon generate millions, if not billions, of cells. Hence, there is a growing need for scalable methods.
