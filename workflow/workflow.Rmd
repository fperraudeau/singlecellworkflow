...
if(packageVersion("clusterExperiment")<"1.5.0-9007") stop("must have current devel version to avoid bugs")
...
of the Bioconductor packages `scone (>=1.1.2)`, `zinbwave (>=0.99.6)`, and `clusterExperiment (>=1.6)`. We recommend running Bioconductor 3.6 (currently the devel version; see https://www.bioconductor.org/developers/how-to/useDevel/).
...
dataObj <- SummarizedExperiment(assays = list(counts = E),
                           colData = metadata)
dataObj
```

EP: changing 'se' to 'dataObj' because it changes classes. Also can search and replace 'dataObj' and not 'se'.  
FP: good idea.
...
mfilt <- metric_sample_filter(assay(dataObj), 
...
# vars <- rowVars(log1p(assay(dataObj)))
# names(vars) <- rownames(dataObj)
# vars <- sort(vars, decreasing = TRUE)
# dataObj <- dataObj[names(vars)[1:1000],]

#alternative script:
dataObj<-makeFilterStats(dataObj,filterStats="var")
dataObj<-filterData(dataObj,percentile=1000)
```
...
```{r setNormAssay}
assay(dataObj)<-assays(dataObj)$normalizedValues
```

EP: This saves a lot of trouble with the plotHeatmap, etc. The code needs to be fixed so *reorders* the assays, rather than what I've done, which is just replace the first. I don't have the object in front of me to be able to do it (it might make sense for me to add option to different assays in clusterExperiment, but I'm not going to do that right now.)
...
FP: W can now be accessed using reducedDim function. Two functions called reducedDim are loaded in the workflow: one from slingshot and the other one from singleCellExperiment.

EP: I would get rid of the `SingleCellExperiment::` code and just add a `library(SingleCellExperiment)`. Much more comfortable code for most users.
...
W <- SingleCellExperiment::reducedDim(dataObj)
d <- dist(W)
...
```{r addW}
SingleCellExperiment::reducedDim(dataObj,"zinbwave50")<-W
```

EP: If zinbwave already has been updated, this above code is probably not needed since its aready there.  
FP: As the name/type "zinbwave50" is then needed in the RSEC function, we might need to keep this line.
EP: I just called it "zinbwave50". We can replace that with whatever its name is in the results from zinbwave. It needs a name, but it can be anything. 
  
EP: Note that we need to see whether clusterExperiment retains all of the different assays or not. It should in principle, but its not something I've tested. Also, I purposefully didn't give the parameter `nReducedDims` because I think it should automatically use the max saved in object if not designated. If not, let me know. 

EP: Currently isCount=FALSE. Does that make sense for the normalized values, or should we set it to TRUE (it shouldn't affect the clustering, since the clustering is only on the reducedDim slot now, but could affect downstream plots, etc.).  
FP: yes, I think isCount=FALSE makes sense for the normalized values.

EP: We could add back in the mergeClusters step now that we don't have the W in the data slot. I haven't done that yet because we want to just recreate the previous, but it would be reasonable now. It would be based on doing DE of the normalized values, however, because I put them into the main assay slot, so we might need to think about this multiple assay question more seriously. But then again, I don't know the workflow for the DE part, so maybe we want to leave it out.  But it would be nice to show that you can do that for this setting. Once this workflow is running I can come back to it.  
...
EP: I've updated so that the default has >500 colors, so don't need the colPalette command, I hope.
...
dataObj <- addClusters(dataObj, colData(dataObj)$publishedClusters, 
...
plotHeatmap(dataObj, 
            whichClusters = c("primary","PublishedClusters"), clusterFeaturesData = "all",
            clusterSamplesData = "dendrogramValue", breaks = 0.99,
            sampleData = c("Batch", "Experiment"),
            clusterLegend = metaColors, annLegend = FALSE, main = "")
```

EP: publishedClusters is saved as a cluster, so don't need to set it up. Now normalized should be default assay, so not need 'visualizeData'.  
...
plotDimReduce(dataObj,whichCluster="primary",reducedDim="zinbwave50",pch=20,
...
EP: I had wrong name for argument (should have been `reducedDim`), so it went with default which is PCA on the assay(dataObj) slot -- i.e. not using zinbwave. Hopefully better now.
...
# c4[primaryClusterNamed(dataObj) == "c4"] <- "cluster c4"
# boxplot(log1p(assay(se)["Reg3g", ]) ~ primaryClusterNamed(dataObj),
#         ylab = "Reg3g log counts", cex.axis = .8, cex.lab = .8)
plotFeatureBoxplot(dataObj, whichCluster="primary", feature="Reg3g",ylab = "Reg3g log counts", cex.axis = .8, cex.lab = .8)
```
...
EP: I think this plot is of the normalized values (saved in assay(dataObj)). But I see that the previous plot was on the counts. Is that better than normalized value?
...
```{r}
pseudoCe <- dataObj[,!primaryClusterNamed(dataObj) %in% c("-1", "c4")]
X <- reducedDim(pseudoCe,type="zinbwave50")
mds <- cmdscale(dist(X), eig = TRUE, k = 4)
X <- mds$points
lineages <- getLineages(X, clusterLabels = primaryClusterNamed(pseudoCe), start.clus = "c1")
```

EP: created a pseudoCe to subset to the samples for the lineage. Simplifies code enormously and as long as we don't change it, doesn't use additional memory, I think.  
...
colorCl<-convertClusterLegend(pseudoCe,whichCluster="primary",output="matrixColors")
pairs(lineages, type="lineages", col = colorCl)
```

EP: convertClusterLegend does the transformation into vector of colors. However, output may be matrix, in which case need to add a `[,1]` to the above code.
...
orderSamples(pseudoCe)<-order(t,na.last=TRUE)
plotHeatmap(pseudoCe[topgenes,!is.na(t)], clusterSamplesData = "orderSamplesValue", breaks = .99)
```

EP: note the above with do heirarchcial clustering of the top genes. If want them to stay in order given, should give `clusterFeaturesData=topgenes` (I think that is the argument)

The above code should take care of the ordering and NA values. Note that subseting a ClusterExperiment object is suppose to also subset the orderSamples value correctly. Let me know if that is not the case. 
