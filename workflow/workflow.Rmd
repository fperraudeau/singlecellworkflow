...
Finally, for computational efficiency, we retain only the 1,000 most variable genes. This seems to be a reasonnable choice for the illustrative purpose of this workflow, as we are able to recover the biological signal found in the published analysis ([@Fletcher2017]). In general, however, we recommend care in selecting an approapriate gene filtering scheme for an actual data analysis.

[FP: check. SD: I rephrased the above paragraph.]
...
The distribution of cells in our workflow's clustering overall agrees with that in the original published clustering (Figures \@ref(fig:addPublishedClusters) and \@ref(fig:comparePublished)), the main difference being that several of the published clusters were merged into single clusters. This discrepancy is likely caused by the fact that we performed dimensionality reduction using ZINB-WaVE and provided the resulting `W` matrix as input to RSEC instead of performing dimensionality reduction on the normalized counts within the `RSEC` function. 

[FP: check. SD: Rephrased a little.]
...
cl = our_cl[!our_cl %in% c("-1", "c7")]
pal = pal[!names(pal) %in% c("-1", "c7")]
X <- W[!our_cl %in% c("-1", "c7"), ]
...
y <- assays(se)$normalizedValues[, !our_cl %in% c("-1", "c7")]
