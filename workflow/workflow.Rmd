...
```{r barplotOurs, fig.cap="RSEC: Barplot of number of cells per cluster, for our workflow's clustering."}
plotBarplot(ceObj, legend = F) 
```


The distribution of cells in our workflow's clustering overall agrees with that in the original published clustering (Figures \@ref(fig:addPublishedClusters) and \@ref(fig:comparePublished)), the main difference being that several of the published clusters were merged here into single clusters. This discrepancy is likely caused by the fact that we performed dimensionality reduction using ZINB-WaVE and provided the resulting `W` matrix as input to `RSEC` instead of performing dimensionality reduction on the normalized counts within the `RSEC` function. 

```{r addPublishedClusters, fig.cap="RSEC: Barplot of number of cells per cluster, for original published clustering."}
...
```{r comparePublished, fig.cap="RSEC: Comparison between the clustering found using our workflow and the original published clustering."}
