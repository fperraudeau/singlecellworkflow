...
if(packageVersion("clusterExperiment")<"1.3.2") stop("must have current develop version to avoid bugs")
...
Finally, we retain only the 1,000 most variable genes. It speeds up the computations and allows to remove genes with low biological signal. As the 1,000 most variable genes allow to recover the biological signal found in the published analysis ([@Fletcher2017]), we think it is a reasonable choice for this particular analysis. We recommend, however, to explore the number of genes required to extract the biological signal in an actual data analysis.
...
```{r barplotOurs, fig.cap="Barplot of number of cells per cluster, for our workflow's clustering."}
plotBarplot(ceObj, legend = F) 
```

The distribution of cells in our workflow's clustering is in agreement with the distribution of cells in the original published clustering (Figures \@ref(fig:addPublishedClusters) and \@ref(fig:comparePublished)) where the major difference is that in the published analysis several clusters have been merged into single clusters in our workflow's clustering. This difference is likelihood to be caused by the fact that we performed the dimensionality reduction step using the ZINB-WaVE model and input the W matrix to the RSEC function instead of performing the dimensionality reduction on the normalized counts within the RSEC function, resulting
