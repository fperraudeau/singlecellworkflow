...
- make sure versions package ok and pushed to bioc (DR, EP, FP),
...
- try running workflow with more genes (DR),
...
of the Bioconductor packages `scone (>=1.1.2)`, `zinbwave (>=0.99.6)`, and `clusterExperiment (>=1.3.2)` [DR: Elizabeth, please check!]. We recommend running Bioconductor 3.6 (currently the devel version; see https://www.bioconductor.org/developers/how-to/useDevel/).
...
stopifnot(sum(is.infinite(norm))==0)
norm[1:3,1:3]
```
...
[FP: we should add a comment on cluster 7.]
...
plotBarplot(ceObj,whichClusters=c("combineMany","publishedClusters"), legend = F) 
```
