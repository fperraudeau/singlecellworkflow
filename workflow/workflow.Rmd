...
if(packageVersion("clusterExperiment")<"1.5.0-9005") stop("must have current devel version to avoid bugs")
...
EP: We could add back in the mergeClusters step now that we don't have the W in the data slot. I haven't done that yet because we want to just recreate the previous, but it would be reasonable now.
...
EP: Note that we need to see whether clusterExperiment retains all of the different assays or not. It should in principle, but its not something I've tested.

```{r rsec_50}
print(system.time(dataObj <- RSEC(dataObj, k0s = 4:15, alphas = c(0.1),
                                betas = 0.8, dimReduce="zinbwave50",
...
plotDimReduce(dataObj,whichCluster="primary",dimReduce="zinbwave50",pch=20,
