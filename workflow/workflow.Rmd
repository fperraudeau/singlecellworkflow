...
The most important parameter to choose for the dimensionality reduction step is the number of hidden covariates to be included in the model, i.e., the number of columns of the hidden matrix `W`, which we denote by `K`. As with any dimensionality reduction technique, choosing the number of factors `K` is far from trivial. There is no "one-fits-all" solution and the researcher may want to try a few different values of `K` and compare the results.

The `zinbwave` package provides two functions that may be used to guide the choice of `K`. The functions `zinbAIC` and `zinbBIC` compute the Akaike information criterion (AIC) and the Bayesian information criterion (BIC), respectively, that can be used to compare models with different values of `K` (smaller values of AIC and BIC indicate a better fit).
...
Depending on the application and on the interpretation of the latent space, we may want to include only a few or several factors. Here, we hope that our reduced space will contain all the biological signal of interest, and selecting a large number of factors seems preferable. The ZINB-WaVE model can also be used to infer unknown normalization factors (similar to RUV [@risso2014normalization]); in such case, a smaller value of `K` would be more appropriate.
